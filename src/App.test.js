...
test('fetching data message shows if show is not defined', ()=> {
...
test('image,title, summary, dropdown dont show before fetch ', () => {
  const dropdown = screen.queryByTestId('dropdown');
  const image = screen.queryByAltText(/stranger things/i)
  const title = screen.queryByText(/stranger things/i);
  const summary = screen.queryByText(/a love letter/i, {exact: false});
